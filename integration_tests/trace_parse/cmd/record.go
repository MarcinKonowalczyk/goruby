...
func main() {
	...
	trimmed, err := utils.TrimPathSuffix(package_path, "trace_parse/cmd")
	if err != nil {
		fmt.Println("Error:", err)
		os.Exit(1)
	}

	test_files_folder := path.Join(trimmed, "trace_parse", TEST_FILES_FOLDER)
	...
	for _, test_file := range test_files {
		...
		out = fmt.Sprintf(HEADER, "trace_parse/cmd/record.go") + "\n" + out

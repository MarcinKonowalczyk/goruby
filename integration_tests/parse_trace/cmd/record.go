...
func main() {
	...
	trimmed, err := utils.TrimPathSuffix(package_path, "parse_trace/cmd")
	if err != nil {
		fmt.Println("Error:", err)
		os.Exit(1)
	}

	test_files_folder := path.Join(trimmed, "parse_trace", TEST_FILES_FOLDER)
	...
	for _, test_file := range test_files {
		...
		out = fmt.Sprintf(HEADER, "parse_trace/cmd/record.go") + "\n" + out
